...
evoked.plot_topomap(
    ...
    average=1.0,
    ch_type="eeg",
    colorbar=False,   # colorbar 自己加，避免和自定义 axes 冲突
    axes=axes,
    show=False
)

# 任意自定义标题（比如换成全局时间 10-20 s）
titles = [f"{10+i}-{11+i} s" for i in range(10)]
for ax, title in zip(axes, titles):
    ax.set_title(title, fontsize=11)

# 公共 colorbar：从第一个子图取回 mappable
cbar = fig.colorbar(axes[0].images[0], ax=axes, fraction=0.015, pad=0.02)
cbar.set_label("Mean EEG amplitude (µV)")
