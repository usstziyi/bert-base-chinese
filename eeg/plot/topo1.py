...
start_sample = int(
    segments_df.iloc[segment_index]["start_sample"]
)

end_sample = int(
    segments_df.iloc[segment_index]["end_sample"]
)

# 直接从原始 Raw 裁剪
segment_raw = raw.copy().crop(
...
print(
    f"duration: "
    f"{segment_raw.n_times / sfreq:.3f} s"
)
...
spectrum = segment_raw.compute_psd(
    method="welch",
    fmin=1,
    fmax=30,
    picks="eeg",
